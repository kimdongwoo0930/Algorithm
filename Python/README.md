...
반복적인 출력을 할 경우 `print()`보다 `sys.stdout.write()`를 사용하는 게 좋다.
...
- `words.items()` : (단어, 길이) 형태의 튜플 리스트
- `lambda x: x[1]` : 튜플의 두 번째 값(단어 길이)을 기준으로 정렬
...
기능은 동일하지만, 한 번만 쓰는 함수라면 lambda가 더 간결하다.
